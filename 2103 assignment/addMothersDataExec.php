...
$check = "select * from Mother_births_by 
        where birth_year = '$birth_year' and child_gender ='$child_gender' and race = '$race' and mth = '$mth' and age_group = '$age_group'";
...
if($user['accountType'] == 'Admin'){
    ...
    if($row){
        echo 'Record exists. Update instead...';
        //livebirths++
//        $update_live_births = "update Mother_births_by set live_births = live_births + 1 
//                where birth_year = '$birth_year' and child_gender ='$child_gender' and race = '$race' and
//                mth = '$mth' and age_group = '$age_group'";  
//        $result_update = sqlsrv_query($conn, $update_live_births);
        echo 'Add successful!'; 
    }// end if
    else{
        ...
        if($row_infants){
            $update_live_births = "insert into Mother_births_by (birth_year, child_gender, race, mth, age_group, live_births) values
                (?, ?, ?, ?, ?, ?)";
            $vals = array($birth_year, $child_gender, $race, $mth, $age_group, $live_births);
            ...
            $update_log = "insert into AdminLog_updates_on (userID, birth_year, entity, new_value, update_type, attribute)
                values(?, ?, ?, ?, ?, ?)";
            $log_result = sqlsrv_query($conn, $update_log, array($user['userID'], $birth_year, 'Mother_births_by', implode(", ", $all_of_new_val),  
                'insert', implode(", ", $all_of_attribute)));
//            if(!$log_result){
